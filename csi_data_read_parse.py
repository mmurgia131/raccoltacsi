...
DATA_COLUMNS_NAMES = ["time","type", "id", "mac", "rssi", "rate", "sig_mode", "mcs", "bandwidth", "smoothing", "not_sounding", "aggregation", "stbc", "fec_coding",
...
def csi_data_read_parse(port: str, baudrate: int, csv_writer, log_file_fd):
    ...
    while True:
        ...
        csi_data.insert(0, current_time)

        if len(csi_data) != len(DATA_COLUMNS_NAMES):
            print(f"[DEBUG] LINE ERROR: {strings}")
            print(f"[DEBUG] COLUMNS: {len(csi_data)}, expected: {len(DATA_COLUMNS_NAMES)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--store', dest='store_file', action='store', default='./csi_data_senzapersone.csv',
    ...
    parser.add_argument('-b', '--baudrate', dest='baudrate', type=int, default=921600,
                    help="Baudrate for serial communication (default: 921600)")
    ...
    subthread = SubThread(serial_port, file_name, log_file_name,args.baudrate)
    subthread.run()
